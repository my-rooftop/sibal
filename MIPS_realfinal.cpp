...
{   
    ...
    string output_n = Name_output.substr(0, Name_output.size()-2);
    output_n.append(".o");

    // Name_output=filePath.substr(0,filePath.size()-2);
...
//R ?????? ?¥í?????? ??????? ???//??? ??Äî ????? 16???? ?¬à?????? ?????.
string binary_inst(vector<string> s, vector<string> la) // la???????//jformat ?????
{
    ...
    if (s[0]=="addu"||s[0]=="and"||s[0]=="nor"||s[0]=="or"||s[0]=="sltu"||s[0]=="subu")
    {
        ...
        for (int i=1; i<s.size(); i++)
        {
            ...
            {
                if (isdigit(s[i][j])==0)//?¬Ù???? ????? ??¡Æ? ???? 0x?¬Ù?, ???¥ï????? ?¬Ù? ????????
                {
                    s[i].erase(j,1);
                    j--;
                }
    ...
    for(int i=start; i<y; i++)
    {
        ...
        else
        {
            ...
            {   
                ...
                lba.push_back(hex_to_dec(nDec));//[i]??¡Æ?? ?????? word?? ??? ???????.                 
            }
            nDec+=4; 
        } 
    }
...
void inst_lb_change(vector<string> &s, vector<string> lb, vector<string> lb_ad)//?¥í???????? label?? ?????????
{
    // cout<<"s[i][0]"<<s[0]<<endl;
    for (int i=1; i<s.size(); i++)
    {
